...
class TestVectorStoreReal:
    ...
    def test_search_returns_results(self):
        ...
        for i, (doc, meta) in enumerate(zip(results.documents, results.metadata)):
            print(
                f"  [DIAG] Result {i}: course={meta.get('course_title')}, lesson={meta.get('lesson_number')}, dist={results.distances[i]:.3f}"
            )
            print(f"  [DIAG]   Snippet: {doc[:100]}...")
        assert not results.error, f"Search returned error: {results.error}"
        assert (
            not results.is_empty()
        ), "Search returned ZERO results for 'What is computer use?'"
...
class TestCourseSearchToolReal:
    ...
    def test_execute_returns_content(self):
        ...
        assert (
            "error" not in result.lower() or "[" in result
...
class TestAPIKeyConfig:
    ...
    def test_api_key_is_set(self):
        ...
        print(
            f"  [DIAG] API key prefix: {config.ANTHROPIC_API_KEY[:10]}..."
            if config.ANTHROPIC_API_KEY
            else "  [DIAG] API key is EMPTY"
        )
        assert (
            config.ANTHROPIC_API_KEY
        ), "ANTHROPIC_API_KEY is empty — AI generator will fail"
        assert (
...
class TestFullPipelineDiagnostic:
    ...
    def test_full_query_returns_answer(self):
        ...
        try:
            ...
            assert (
                "error" not in response.lower() or len(response) > 50
