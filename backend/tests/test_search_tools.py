...
class TestCourseSearchToolExecute:
    ...
    def test_execute_sources_with_lesson_links(self, search_tool, mock_vector_store, sample_search_results):
        ...
        assert any("[Introduction to MCP - Lesson" in s for s in search_tool.last_sources)

    def test_execute_returns_error_string(self, search_tool, mock_vector_store, error_search_results):
    ...
    def test_execute_empty_with_both_filters(self, search_tool, mock_vector_store):
        ...
        result = search_tool.execute(query="something", course_name="MCP", lesson_number=3)
    ...
    def test_execute_passes_params_to_store(self, search_tool, mock_vector_store):
        ...
        search_tool.execute(query="What is MCP?", course_name="MCP Course", lesson_number=2)
...
class TestToolManager:
    ...
    def test_tool_manager_execute_dispatches(self, tool_manager, mock_vector_store, sample_search_results):
        """execute_tool dispatches to the correct tool's execute method."""
        mock_vector_store.search.return_value = sample_search_results

        result = tool_manager.execute_tool("search_course_content", query="What is MCP?")
    ...
    def test_tool_manager_get_and_reset_sources(self, tool_manager, mock_vector_store, sample_search_results):
